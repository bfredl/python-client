...
glib_major, glib_minor = [int(x) for x in GLib._version.split('.')]
if glib_major < 2 or glib_minor < 32:
    GLib.threads_init()
...
class GtkUI(object):
    ...
    def start(self, bridge):
        ...
        drawing_area.connect('draw', self._on_gtk_draw)
        window = Gtk.Window()
    ...
    def _nvim_clear(self):
        self._clear_region(0, self._screen.rows, 0, self._screen.columns)
        self._screen.clear()
    ...
    def _on_gtk_draw(self, wid, cr):
        ...
        cr.paint()
        cr.restore()
        if self._cursor_enabled and self._show_cursor:
    ...
    def _on_gtk_configure(self, widget, event):
        def resize(*args):
            self._resize_timer_id = None
            ...
            if self._screen.columns == columns and self._screen.rows == rows:
                return
            self._bridge.send_resize(columns, rows)
    ...
    def _on_gtk_quit(self, *args):
        self._bridge.disconnect()

    def _on_gtk_key(self, widget, event, *args):
    ...
    def _on_gtk_button_release(self, widget, event, *args):
        self._pressed = None

    def _on_gtk_motion_notify(self, widget, event, *args):
    ...
    def _flush(self):
        ...
        buf = []
        bold = False
        for col, text, attrs in self._screen.slice(row, startcol, endcol):
    ...
    def _get_pango_attrs(self, attrs):
        ...
        if rv is None:
            n = {
                'foreground': _split_color(self._foreground),
                'background': _split_color(self._background),
            }
            if attrs:
                for k, v in attrs.items():
                    if k in ['foreground', 'background']:
                        ...
                        n['underline'] = 'single'
            c = dict(n)
            c['foreground'] = _invert_color(*_split_color(self._foreground))
            c['background'] = _invert_color(*_split_color(self._background))
